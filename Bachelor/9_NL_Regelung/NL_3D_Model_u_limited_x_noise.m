u_sat_on  = true;
x_offset  = zeros(9,1);
x_noise   = [ones(3,1)*0.0008; zeros(6,1)];
x_noise   = [ones(3,1)*0.0003; zeros(6,1)];
stop_time = 4;
...
title('Verlauf x_g, limitierte Stellgröße, x_{noise}');
...
for n = 1:it
    ...
    title('Verlauf u_{R1}, limitierte Stellgröße, x_{noise}');
    subplot(3,1,2);
    plot(t_x{n}, ur2, 'black'); grid; hold on;
    xlabel('Zeit [s]'); ylabel('u_{R2} [rad/sec]');
    title('Verlauf u_{R2}, limitierte Stellgröße, x_{noise}');
    subplot(3,1,3);
    plot(t_x{n}, ur3, 'black'); grid; hold on;
    xlabel('Zeit [s]'); ylabel('u_{R3} [rad/sec]');
    title('Verlauf u_{R3}, limitierte Stellgröße, x_{noise}');
end
...
for n = 1:it
    ...
    title('Verlauf u_1, limitierte Stellgröße, x_{noise}');
    subplot(3,1,2);
    plot(t_u{n}, u2, 'black'); grid; hold on;
    xlabel('Zeit [s]'); ylabel('u_2 [Nm]');
    title('Verlauf u_2, limitierte Stellgröße, x_{noise}');
    subplot(3,1,3);
    plot(t_u{n}, u3, 'black'); grid; hold on;
    xlabel('Zeit [s]'); ylabel('u_3 [Nm]');
    title('Verlauf u_3, limitierte Stellgröße, x_{noise}');
end